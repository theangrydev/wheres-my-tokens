...
|---|---:|---:|
| **cache creation only** | **0.8051** | 0.9224 |
| cost + cache creation | 0.8044 | 0.9238 |
| all four token types | 0.8029 | **0.9572** |
| cost-weighted (API $) | 0.7416 | 0.8157 |
| cache **reads** only | 0.6643 | 0.7451 |

*(AUC. 88 hit / 2,335 control windows for the 5-hour fit; 44 / 354 for the weekly one.)*

- **Cache creation is the limit driver, in both buckets.** For the 5-hour limit it alone is the *best* model — adding the other three token types makes it slightly worse. For the weekly limit it carries a standardised coefficient of **+3.2**, roughly double the next largest.
- **Dollar cost is not what's being metered.** Pair cost with cache creation and cost's coefficient goes **negative** in both fits: once you know how much cache a window wrote, spending *more* predicts *fewer* limit hits. Cache creation is worth +0.06 AUC over cost alone on the 5-hour limit and +0.11 on the weekly one.
- **Cache reads look like the *good* state, not a risk.** Controlled for cache creation their coefficient is -0.010 (5-hour) and -0.066 (weekly). They are ~97% of all tokens, so on their own they correlate with everything, but once cache creation is in the model they carry essentially nothing. A window heavy in reads and light in creation is a long warm session — which is exactly the state to aim for.
...
**5-hour session limit** — one feature does almost all the work:

![Logistic Regression, 5-hour limit](img/5h/logistic_regression.png)

**Weekly limit** — a genuine weighted combination, and cache creation dominates it:

![Logistic Regression, weekly limit](img/1w/logistic_regression.png)

*(Run `wheres-my-tokens analyze` to reproduce both on your own data)*
...
Grey dots are windows that did *not* hit a limit, so the gap between the clouds is the thing to read. Note how much lower the weekly ceiling sits relative to the traffic underneath it.

![Budget Timeline, 5-hour limit](img/5h/budget_timeline.png)
![Budget Timeline, weekly limit](img/1w/budget_timeline.png)

#### Feature correlations with limit hits
![Calibration Scatter, 5-hour limit](img/5h/calibration_scatter.png)

*(Note: The Logistic Regression charts shown earlier are also generated by this command. Each window writes a full set to `output/<window>/`; only the 5-hour scatter is reproduced here.)*
